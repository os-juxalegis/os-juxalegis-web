    ...
    if user_prompt:
        ...
        if not es_duplicado:
            ...
            if anthropic and CLAUDE_API_KEY and not CLAUDE_API_KEY.startswith("TU_CLAVE"):
                ...
                # ==============================================================
                # NUEVO MANEJO DE ERROR ELEGANTE (Sin asustar con JSONs)
                # ==============================================================
                if not exito:
                    respuesta_completa = (
                        "⚠️ **Aviso de Infraestructura (Anthropic):**\n\n"
                        "El sistema intentó conectarse correctamente a la API, pero los servidores de Anthropic rechazaron la solicitud (Error 404 - Modelo no autorizado).\n\n"
                        "**Verificaciones requeridas en su cuenta administrativa de Anthropic Console:**\n"
                        "- Verifique tener **saldo acreditado** en la pestaña de *Billing* (se requieren mínimo $5 USD para habilitar estos modelos).\n"
                        "- Asegúrese de que su clave API no pertenezca a una capa gratuita (Free Tier) bloqueada para uso productivo.\n"
                    )
                    contenedor_respuesta.markdown(respuesta_completa)
