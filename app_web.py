    ...
    with col2:
        ...
        with st.form("form_login"):
            ...
            if btn_ingresar:
                ...
                if email_limpio in CORREOS_AUTORIZADOS:
                    st.session_state.autenticado = True
                    st.session_state.usuario_email = email_limpio
                    st.rerun()
                else:
                    st.error("Credenciales no autorizadas en el sistema.")
    st.stop()
...
if vista == "chat":
    ...
    if (submit_clicked or user_prompt) and user_prompt.strip():
        ...
        with st.chat_message("assistant", avatar=None):
            st.markdown(f"<span class='msg-header-assistant'>{alias_display}:</span>", unsafe_allow_html=True)
